...
### Prerequisites:
    ...
    Our scripts were run on FAS Research Computing Cluster https://www.rc.fas.harvard.edu/
    You will have to modify some of the scripts to run on a local computer.
...
### Library analysis
  - Extract, count barcodes and remove singletons. <br>
   ```
   `perl BarcodeReaderV.plx library.fastq_file | awk '{print $3}' | sort | uniq -c | awk '$1 > 1 {print $2 "\t" $1}' | sort -k2 -nr > LIB065162_primary_reads_sort_count_remove_singletons.tsv`
   ```
   - Find inflection point and outputs plot. <br>
...
### WGS Barcoding 7mer
  - Extract and process 7 base barcodes.
      - WGSBarcodeCounter7mer.plx
      - SumBarcodes7mer.plx <br>
      - RemoveSequencingErrors.plx
  `sbatch barcodes7mer.sh sample_list.tsv` <br>
...
### Final curation
All outputs were subject to manual curation. 
