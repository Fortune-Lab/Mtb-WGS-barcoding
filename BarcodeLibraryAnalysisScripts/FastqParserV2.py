...
try:
    for read1, read2 in zip(parse_fastx_file("LIB065162_GEN00299530_G2G_S1_L001_R1_001.fastq.gz"), parse_fastx_file("LIB065162_GEN00299530_G2G_S1_L001_R2_001.fastq.gz")):
        pattern = r'(GCGGCCGCGAATTCCG[AG])([ATCG]{16,19})([GC]AATTCGATGGCCT)'
        print(read1.seq, read2.seq)
        match = re.search(pattern, read1.seq)
        if match:
            if match.group(2) in barcodes:
                barcodes[match.group(2)] += 1
                print(barcodes[match.group(2)])
            else:
                barcodes[match.group(2)] = 1
        #print(record.id)
        #print(record.seq)
